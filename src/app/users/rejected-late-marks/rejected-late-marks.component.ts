...
export class RejectedLateMarksComponent implements OnInit {
  ...
  constructor(private router:Router,private api:RejectedLateMarksService,private monthandyear:MonthYearService,private modalService: BsModalService,public toastr: NotificationService) {
    var filteredData=monthandyear.getFilterData();
    this.rejectedLateMarksData.start_date = filteredData[0].firstDay;
    this.rejectedLateMarksData.end_date = filteredData[0].lastDay;
    this.rejectedLateMarksForm = new FormGroup({
  ...
  ngOnInit() {
    var start_date=moment(this.rejectedLateMarksData.start_date).format('DD/MM/YYYY');
